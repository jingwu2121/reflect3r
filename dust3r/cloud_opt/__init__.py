...
def global_aligner(dust3r_output, device, min_conf_thr=3, preset_poses=None, mode=GlobalAlignerMode.PointCloudOptimizer, allow_pw_adaptors=False, **optim_kw):
    # extract all inputs
    view1, view2, pred1, pred2 = [dust3r_output[k] for k in 'view1 view2 pred1 pred2'.split()]
    # build the optimizer
    if mode == GlobalAlignerMode.PointCloudOptimizer:
        net = PointCloudOptimizer(view1, view2, pred1, pred2, **optim_kw, min_conf_thr=min_conf_thr, allow_pw_adaptors=allow_pw_adaptors, preset_poses=preset_poses).to(device)
    elif mode == GlobalAlignerMode.ModularPointCloudOptimizer:
        Warning('min_conf_thr is ignored for ModularPointCloudOptimizer')
        net = ModularPointCloudOptimizer(view1, view2, pred1, pred2, **optim_kw).to(device)
    elif mode == GlobalAlignerMode.PairViewer:
        net = PairViewer(view1, view2, pred1, pred2, **optim_kw, min_conf_thr=min_conf_thr).to(device)
    else:
