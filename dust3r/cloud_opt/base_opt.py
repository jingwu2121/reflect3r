...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        self.im_conf = self._compute_img_conf(pred1_conf, pred2_conf) # make sure there is no negative confidence
        ...
        self.mirror_masks = None
        if 'mask' in view1 and 'mask' in view2:
            mirror_masks = [torch.zeros((3,)+hw) for hw in self.imshapes]
            for v in range(len(self.edges)):
                idx = view1['idx'][v]
                mirror_masks[idx] = view1['mask'][v]
                idx = view2['idx'][v]
                mirror_masks[idx] = view2['mask'][v]
            self.mirror_masks = mirror_masks
    ...
    def _set_im_pose(self, poses, idx, R, T=None, scale=None, force=False):
        # all poses == cam-to-world
        pose = poses[0]
        if not (pose.requires_grad or force):
            return pose
    ...
    def _set_pose_no_grad(self, poses, idx, R, T=None, scale=None, force=False):
        # all poses == cam-to-world
        pose = poses[idx]
        ...
        assert pose.requires_grad == False
        return pose
    ...
    def get_pts3d(self, raw=False, use_mirror_masks=False):
        res = self.depth_to_pts3d(use_mirror_masks)
        if not raw:
...
def global_alignment_loop(net, lr=0.01, niter=300, schedule='cosine', lr_min=1e-6, final_edge=None):
    ...
    if verbose:
        with tqdm.tqdm(total=niter) as bar:
            while bar.n < bar.total:
                loss, lr, _ = global_alignment_iter(net, bar.n, niter, lr_base, lr_min, optimizer, schedule, final_edge)
                bar.set_postfix_str(f'{lr=:g} loss={loss:g}')
                bar.update()
                loss_history.append(loss)
    else:
        for n in range(niter):
            loss, _, _ = global_alignment_iter(net, n, niter, lr_base, lr_min, optimizer, schedule, final_edge)
            loss_history.append(loss)
    return loss, lr, loss_history


def my_global_alignment_iter(net, cur_iter, niter, lr_base, lr_min, optimizer, schedule, final_edge=None):
    t = cur_iter / niter
    if schedule == 'cosine':
    ...
    # Update the mirror view pose
    (main_view_idx, mirror_view_idx) = final_edge
    im_poses = net.get_im_poses()
    cam2world_main = im_poses[main_view_idx]

    # Get the mirror plane
    proj_pts3d_main_view = net.get_pts3d(raw=False)[main_view_idx]
    main_view_mask = net.main_view_mask # Now it is the outside mask
    ...
    net._set_pose_no_grad(net.im_poses, mirror_view_idx, cam2world_mirror)

    return float(loss), lr, net

def global_alignment_iter(net, cur_iter, niter, lr_base, lr_min, optimizer, schedule, final_edge=None):
    t = cur_iter / niter
    if schedule == 'cosine':
        lr = cosine_schedule(t, lr_base, lr_min)
    elif schedule == 'linear':
...
def my_global_alignment_iter_poseloss(net, sigma, cur_iter, niter, lr_base, lr_min, optimizer, schedule, final_edge=None, intermediate_save_dir=None):
    t = cur_iter / niter
    if schedule == 'cosine':
        lr = cosine_schedule(t, lr_base, lr_min)
    elif schedule == 'linear':
