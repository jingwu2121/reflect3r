...
class PairViewer (BasePCOptimizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(self.n_imgs):
            ...
            # Save mask as binary image using PIL
            # msk_img = Image.fromarray((msk * 255).astype('uint8'))
            # msk_img.save(f'mask_{i}.png')
        ...
        if confs[0] > confs[1]:
            ...
            if 'mask' in self.view_i.keys():
                self.mirror_masks = [self.view_i['mask'][1], self.view_j['mask'][1]]
            else:
                self.mirror_masks = [None, None]
        else:
            ...
            if 'mask' in self.view_j.keys():
                self.mirror_masks = [self.view_j['mask'][0], self.view_i['mask'][0]]
            else:
                self.mirror_masks = [None, None]
    ...
    def get_depthmaps(self, raw=False, use_mirror_masks=False):
        if use_mirror_masks:
            depth = []
            for d, mirror_mask in zip(self.depth, self.mirror_masks):
                depth.append(d.to(self.device) * mirror_mask[0].to(self.device))
        else:
    ...
    def depth_to_pts3d(self, use_mirror_masks=False):
        pts3d = []
        colors = []
        for d, img, conf_mask, mirror_mask, intrinsics, im_pose in zip(self.depth, self.imgs, self.conf_mask, self.mirror_masks, self.get_intrinsics(), self.get_im_poses()):
            ...
            # if use_mirror_masks:
            #     # import PIL
            #     # mirror = PIL.Image.fromarray((self.view_j['mask'].numpy()[1][0] * 255).astype(np.uint8))
            #     # mirror.save('mirror.png')
            #     # breakpoint()
            #     mirror_mask = mirror_mask[0].numpy()
            #     mask = conf_mask * mirror_mask
            #     pts_masked = pts[mirror_mask]
            ...
            # colors.append(torch.from_numpy(img_masked).to(device=self.device))
