...
def load_images(folder_or_list, size, mask_or_list=None, square_ok=False, verbose=True):
    ...
    if use_mask:
        for img_path, mask_path in zip(folder_content, mask_content):
            ...
            #     bg = PIL.Image.new('RGBA', img.size, (128, 128, 128))
            #     if img.mode == 'P':
            #         img = img.convert('RGBA')
            #     # Composite the image onto the white background
            #     img = PIL.Image.alpha_composite(bg, img)
            img = img.convert('RGB')
            mask = exif_transpose(PIL.Image.open(os.path.join(root, mask_path)))
            ...
            mask = np.array(mask) 
            mask = mask > 128
            # img.save(f'temp/{img_path.split("/")[-1]}')
            # breakpoint()
            if verbose:
            ...
            imgs.append(dict(img=ImgNorm(img)[None], true_shape=np.int32(
                [img.size[::-1]]), idx=len(imgs), instance=str(len(imgs)), mask=mask[None][None]))
    else: 
